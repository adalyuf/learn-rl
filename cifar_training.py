...
def model_train(config=config_defaults, use_last=False):
    with wandb.init(project=config.wandb_project, config=config, dir=f'wandb/{dataset_name}/{config.model_choice}'):
        ...
        if model_choice == "ExampleNet":
            ...
            model = NewAlexNet(bn_before_relu=config.bn_before_relu, num_classes=10, dropout_rate=config.dropout_rate)
        ...
        sample_images = []
        for i in range(1,17):
            image = wandb.Image(images[i], caption=classes[labels[i]])
        ...
        for epoch in range(1, epochs+1):
            ...
            if validation_loss < validation_loss_min:
                ...
                torch.save(model.state_dict(), f'models/cifar/{model_choice}/model.pt')
...
def model_eval(model_choice, epochs=None, lr=None, bn_before_relu=True, dropout_rate=0.25):
    if model_choice == "ExampleNet":
        ...
        model = NewAlexNet(bn_before_relu=bn_before_relu, num_classes=10, dropout_rate=dropout_rate)
    ...
    criterion = nn.CrossEntropyLoss()
    model.load_state_dict(torch.load(f'models/cifar/{model_choice}/model.pt'))
